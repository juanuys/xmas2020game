...
class Hero extends Entity {
	...
	public var hasPlayedNoteC = false;
				...
				if( obj != null ) {
					...
					case Square1, Square2, Square3, Wings:
						...
						case Wings:
							if (!hasPlayedNoteC) {
								hasPlayedNoteC = true;
								hxd.Res.sfx.noteC.play(0.5);
							} else {
								hxd.Res.sfx.noteCAlt.play(0.5);
							}
						default:
						}
						obj.carried = true;
	...
	override function update(dt:Float) 	{
		...
		if (hasWings) {
			...
			spr.y -= 10;
			spr.x -= (Math.sin(time) + 1) * (1 + flying) - 3 + flying * 8;
		}
	}
